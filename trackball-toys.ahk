...
; Press Ctrl+Alt+D to toggle side button disable/enable
^!d::
    SideButtonsDisabled := !SideButtonsDisabled
    if (SideButtonsDisabled) {
        ToolTip, Side buttons DISABLED
        SetTimer, RemoveToolTip, 2000
    } else {
        ToolTip, Side buttons ENABLED
        SetTimer, RemoveToolTip, 2000
    }
    return

; Conditional disable based on toggle
XButton1::
    if (SideButtonsDisabled) {
        return  ; Do nothing if disabled
    } else {
...
; - Ctrl+Alt+S: Show side button status
; - Ctrl+Shift+S: Toggle trackball scrolling on/off
; - Ctrl+Shift+V: Open VSCode in current Explorer folder
; - Escape: Exit script
; =============================================================================
